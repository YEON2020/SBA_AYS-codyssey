...
print("\n--- 중복 조건 확인 ---")
# 같은 감독의 영화
duplicate_directors = df.groupby('감독')['제목'].apply(list).loc[lambda x: x.map(len) > 1]
...
# 1. 특정 연도 필터링 (예: 2023년)
year_filter = 2023
movies_by_year = df[pd.to_datetime(df['개봉일'], errors='coerce').dt.year == year_filter]
print(f"{year_filter}년에 개봉한 영화:")
print(movies_by_year[['제목', '개봉일']])

# 2. 관객의 감정 상태에 따른 추천
print("--- 관객의 감정상태 기반 추천 ---")
mood_genre_map = {
    '행복': ['코미디'],
    '슬픔': ['드라마'],
    '즐거움': ['액션', '스릴러'],
...
current_mood = '행복'  # 감정 입력 예시
if current_mood in mood_genre_map:
    mood_genres = mood_genre_map[current_mood]
    mood_recommendations = df[df['장르'].apply(lambda x: any(genre in x for genre in mood_genres))]
    print(f"감정 상태 '{current_mood}'에 적합한 영화:")
    print(mood_recommendations[['제목', '장르']])
else:
    print(f"'{current_mood}' 감정에 해당하는 추천이 없습니다.")
